...
**Used by:** /team page — hero photo, individual care-team member portraits (also referenced by /care-team/{slug} when that route ships).

## How to add an image
1. Drop the file in this folder (recommended naming: `hero.webp`, `section-{name}.webp`, etc.)
2. Reference it in code as `/team/{filename}` — e.g. `<Image src="/team/hero.webp" />`
