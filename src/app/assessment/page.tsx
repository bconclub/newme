...
export default function AssessmentPage() {
  ...
  // ── STATS SCREEN ──────────────────────────────────────────────────────────────
  if (screen === 'stats') {
    // Group condition cards into pairs — each pair is one carousel page
    const cardPages: (typeof CONDITION_CARDS)[] = []
    for (let i = 0; i < CONDITION_CARDS.length; i += 2) cardPages.push(CONDITION_CARDS.slice(i, i + 2))

    return (
      <div style={{ background: '#173B39', color: '#fff', position: 'relative', height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <AtmosphereBg />
